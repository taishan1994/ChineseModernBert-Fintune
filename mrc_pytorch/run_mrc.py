...
def evaluate(model, args, eval_examples, eval_features, device, global_steps, best_f1, best_em, best_f1_em):
    ...
    for f in eval_features:
        input_ids = [0 if i is None else i for i in f['input_ids']]
        input_mask = [0 if i is None else i for i in f['input_mask']]
    ...
    for input_ids, input_mask, example_indices in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    if float(tmp_result['F1']) + float(tmp_result['EM']) > best_f1_em:
        ...
        try:
            ...
            model.module.save_pretrained(args.checkpoint_dir)
...
if __name__ == '__main__':
    ...
    for seed_ in args.seed:
        ...
        model = AutoModelForQuestionAnswering.from_pretrained(args.bert_dir, reference_compile=False)
        ...
        for i in range(int(args.train_epochs)):
            ...
            with tqdm(total=steps_per_epoch, desc='Epoch %d' % (i + 1)) as pbar:
                for step, batch in enumerate(train_dataloader):
                    ...
                    input_ids, input_mask, start_positions, end_positions = batch
                    output = model(input_ids, input_mask, start_positions=start_positions, end_positions=end_positions)
                    loss = output.loss
                    if n_gpu > 1:
